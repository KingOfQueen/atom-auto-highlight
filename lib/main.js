      ...
      editor.onDidChangeCursorPosition(event => {
          ...
          try {
              ...
              if(cursorWordChanged){
                  if(maybeVariableName(word) && word.length>=2 ){
                      if(ToggleWordCount>=1){
                          //clear toggles if more than 1 words toggled:
                          this.keywordManager && this.keywordManager.clear();
                          ToggleWordCount = 0
                      }

                      //FIXME: to highlight whole word except prefix matched ones
                      toggle(word)
                      ToggleWordCount += 1
